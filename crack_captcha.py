...
class Cracker():
    ...
    def captcha_cnn(self, w_alpha = 0.01,b_alpha = 0.1):
        ...
        dense_layer = tf.nn.relu(tf.add(tf.matmul(dense_layer,w_d),b_d))
        ...
        out = tf.add(tf.matmul(dense_layer, w_out), b_out)
               ...
               # shuffle set every 500  steps
                ...
                if i % 10 ==0:
                    ...
                    if acc > 0.95:
                        train_writer.close()
                        test_writer.close()
                        saver.save(sess, "Tb2/captcha_recognition_model.ckpt", global_step=i)
                ...
                train_writer.close()
                test_writer.close()
                saver.save(sess, "Tb2/captcha_recognition_model.ckpt", global_step=self.max_steps)

if __name__ =='__main__':
    cracking = Cracker()
    cracking.train_captcha_cnn()